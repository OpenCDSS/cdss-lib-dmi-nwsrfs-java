// ----------------------------------------------------------------------------
// Fcst.java - class for interacting with the NWSRFS fcst program (e.g., process
//			output files).
// ----------------------------------------------------------------------------
...
//					though the TS was "Day". Change filename//					to have the "Day" extension.
// ----------------------------------------------------------------------------
// EndHeader
...
/**
Fill an existing DayTS object with data stored inside an IrregularTS   
object.
...
data section.
@param lineSpace the number of lines between the line that contained the
date value and the beginning of the actual data.
...
*/
private void processData (	BufferedReader br, Hashtable ht, 
				DateTime date, int lineSpace, String format,
				int [] nameFields, int [] idFields,
				int [] dataFields, String filename,
				String units )
throws Exception
{	/////////////////////////////////////////////////////////////
	// Spacing routine -- skip all the lines that occur between the
	// header for this section of data and the actual start of the 
	// data values
	...
	while (true) {
		...
		// List of occurences that mark the valid end of the data.  It
		// appears that the line can be ignored after control returns
		// to the calling code.
		...
		for ( i = 0; i < num; i++) {
			...
			if ( id.equals("") && (i == 0) ) {
				...
				"Fcst.processData", "No ID in data section.  " +
				"File/software problem?  File: \"" + filename +
				"\" line: " + __lineNumber + ":" );
				Message.printWarning ( 2, "Fcst.processData",
					line );
				return;
			}
...
{	/////////////////////////////////////////////////////////////
	...
	String dailyLine = "";			// The first part of the line
						...
						// string.
	int dailyLineSpace = -1;		// The number of spaces between
						// the occurence of dailyLine
						// and the first line containing
						// data (intervening lines are
						// other comments).
	...
	String less24Line = "";			// The first part of the line
						...
						// in this string.
	int less24LineSpace = -1;		// The first part (up to the
						// units of measure) of the line
						// that specifies the units of
						// measure for less than 24 hour
						// data.
	...
	if ( warning.length() > 0 ) {
		Message.printWarning ( 2, "Fcst.writeFile",
...
{	Enumeration en = ht.keys();
	...
	while(en.hasMoreElements()) {
		...
		if ( append && IOUtil.fileExists(filename) ) {			
			...
			dts.addToGenesis("Fcst.processMAPOutput "
					+ "add to file from:");
		}
